...
def preprocess_random_rotate(dataset: UnlabeledDataset, rotation_range: Tuple[int,int] = (-10, 10), batch_size=1000)-> UnlabeledDataset:
    """
    ...
    d = device('cuda:0' if torch.cuda.is_available() else 'cpu')
    result_x = Tensor([])
    bottom, top = rotation_range
    for i, data in enumerate(loader, 0):
        rotation = random.random() * (top-bottom) + bottom
        x = data.to(d)
