...
rotation_augmented_data_PATH = Path(PROCESSED_DATA_PATH / "rotation-augmented-data-last90percent.data")
rotation_augmented_ignorext_data_PATH = Path(PROCESSED_DATA_PATH / "rotation-augmented-ignorext-data-last90percent.data")
...
def run_alex(n_way: int, depth: Tuple[int, int, int], scaled: bool, relabeled: bool, rotation_augment = False, ignorext = False, semisupervised = False):
    print(f"n_way: {n_way}")
    print(f"depth: {depth}")
    training_set = read_train_labeled(DATASET_PATH)
    validation_proportion = 0.1
    trained_alex_net_PATH = TRAINED_ALEX_NET_PATH
    alex_net_WRONG_PRED_PATH = ALEX_NET_WRONG_PREDICTION_PATH
    if rotation_augment:
    ...
    train_unlabeled = read_train_unlabeled(DATASET_PATH)
    train_unlabeled_augmented = preprocess_random_rotate(train_unlabeled)
    if scaled:
        trained_alex_net_PATH = Path(Path(str(trained_alex_net_PATH) + "-scaled"))
        alex_net_WRONG_PRED_PATH = Path(
            Path(str(alex_net_WRONG_PRED_PATH) + "-scaled"))
        train = preprocess_scale_image(train)
        validation = preprocess_scale_image(validation)
        train_unlabeled = preprocess_scale_image(train_unlabeled)
        train_unlabeled_augmented = preprocess_scale_image(train_unlabeled_augmented)
    if relabeled:
        trained_alex_net_PATH = Path(Path(Path(str(trained_alex_net_PATH) + "-relabeled")))
        alex_net_WRONG_PRED_PATH = Path(
            Path(str(alex_net_WRONG_PRED_PATH) + "-relabeled"))
    ...
    alex_net_WRONG_PRED_PATH = Path(alex_net_WRONG_PRED_PATH / f"{n_way}way-depth{depth}")

    train_unlabeled_mixed = UnlabeledDataset(train_unlabeled.x, train_unlabeled_augmented)

    alex = AlexNetClassifier(train, validation, training_ul=train_unlabeled_mixed, n_way=n_way, depth=depth, scaled=scaled, relabeled=relabeled)
    alex.set_optimizer(ADAM_PROFILE)
    if semisupervised:
        trained_alex_net_PATH = Path(
            Path(str(trained_alex_net_PATH) + "-fixmatch"))
        alex_net_WRONG_PRED_PATH = Path(
            Path(str(alex_net_WRONG_PRED_PATH) + "-fixmatch"))
        alex.train_fixmatch(30,
                   batch_size=100,
                   plugins=[
...
if __name__ == '__main__':
    ...
    # load_and_test_NN(AlexNetClassifier, {'n_way': 1, 'depth': (3,4,4), 'scaled': False, 'relabeled': False,}, 26, 13, DATASET_PATH,
    #                  Path(TRAINED_MODELS_PATH / 'alex-nets/1way-depth(3, 4, 4)-semisupervised-on-test-2'))
